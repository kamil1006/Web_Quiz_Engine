...
class TransactionExample {
    ...
    public static long calcSumOfCanceledTransOnNonEmptyAccounts(List<Account> accounts) {
        ...
        List<List<Transaction>> listaList = accountStream.map(Account::getTransactions).collect(Collectors.toList());

        List<Transaction> listaTransakcji = listaList.stream().flatMap(transactions -> transactions.stream()).collect(Collectors.toList());
         listaTransakcji = listaTransakcji.stream().filter(transaction -> transaction.state == State.CANCELED).collect(Collectors.toList());
        long sum = listaTransakcji.stream().mapToLong(f -> f.getSum()).sum();
